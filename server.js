...
require("dotenv").config({
  path: ".env",
}); /*things to keep secret, make sure it is able to use those variables*/
...
); /*store css files in public folder, express will look there, hey express if you're looking for style sheets look in the public folder */
app.use(
  express.urlencoded({ extended: true })
); /*help validate information we're passing back and forth, extended allow us to do send complex items */

mongoose.connect(
  process.env.DB_STRING, //telling mongoose where to look //needed .env!
...
app
  ...
  .get((req, res) => {
    ...
    TodoTask.find({}, (err, tasks) => {
      //find the list of tasks and the one i want to edit
      res.render("edit.ejs", {
